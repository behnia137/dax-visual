...
You add a calculated column to `FactSales` called `Revenue = Sales[Quantity] * Sales[UnitPrice]`. When Power BI processes this at refresh, it creates row context for each row in the table. On row 1, `Sales[Quantity]` resolves to that row's quantity and `Sales[UnitPrice]` resolves to that row's unit price, so the multiplication gives you the revenue for that specific sale. The same column formula runs independently for every row, which is exactly what you want.
